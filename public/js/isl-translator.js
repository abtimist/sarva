    ...
    for (let char of word) {
      if (char.match(/[a-z]/)) {
        ...
        try {
          ...
          await sleep(400); // just wait if even letter fails
        }
      }
    }
  }
...
function playWordVideo(word) {
  return new Promise((resolve, reject) => {
    ...
    // Set video source (using a placeholder robust URL logic)
    // NOTE: Replace this with the actual Open Source ISL project video CDN
    videoEl.src = `https://isl-dummy-cdn.vercel.app/videos/${encodeURIComponent(word)}.mp4`; 
    
    // If the video actually fails to load because the dummy CDN is offline,
    // we handle the error. Since we don't have a real active CDN right now,
    // we simulate playback for MVP demonstration.
    
    const simulatePlayback = true; 
    
    if (simulatePlayback) {
      // Simulate playback time based on word length
      setTimeout(() => resolve(), 800 + (word.length * 100));
    } else {
